...
const HTML_TEMPLATE = (order: OrderNotification, adminLink: string) => `
  <!DOCTYPE html>
  <html>
...
export async function sendNewOrderNotification(order: OrderNotification) {
  const notificationTargets: { apiKey: string; email: string }[] = [
    { apiKey: process.env.RESEND_API_KEY_1, email: process.env.NOTIFICATION_EMAIL_1 },
    { apiKey: process.env.RESEND_API_KEY_2, email: process.env.NOTIFICATION_EMAIL_2 },
  ].filter((t): t is { apiKey: string; email: string } => !!t.apiKey && !!t.email);

  if (notificationTargets.length === 0) {
    console.warn("[email] No Resend accounts configured — skipping notification");
    return;
  }
  ...
  const results = await Promise.allSettled(
    notificationTargets.map(async (target) => {
      const resend = new Resend(target.apiKey);
      await resend.emails.send({
        from: "onboarding@resend.dev",
        to: target.email,
        subject: `Nouvelle commande — ${order.eventTitle}`,
        html: HTML_TEMPLATE(order, adminLink),
      });
    })
  );

  results.forEach((result, i) => {
    const target = notificationTargets[i];
    if (result.status === "rejected") {
      console.error(
        `[email] Failed to send notification to ${target.email} (apiKey index ${i}):`,
        result.reason
      );
    } else {
      console.log(`[email] Notification sent to ${target.email} for order ${order.id}`);
    }
  });
}
